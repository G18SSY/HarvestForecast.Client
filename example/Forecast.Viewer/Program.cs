...
public static class Program
{
    ...
    public static async Task Main()
    {
        ...
        if (person is { })
        {
            ...
            var workingToday = person.WorkingDays.IsActiveOn(DateTime.Today.DayOfWeek);
            var workingLine = workingToday
                ? "and it looks like today is one of your working days!"
                : "and it looks like today is not one of your working days!";
            AnsiConsole.MarkupLine($"[green]{workingLine}[/]");
        }
        else
        {
            AnsiConsole.MarkupLine(
                "[red]We can't check your profile details right now but we can still check your assignments...[/]");
        }
        ...
        {
            AnsiConsole.MarkupLine(
                $"[green]Looks like you've got {assignments.Count} assignment{(assignments.Count == 1 ? "" : "s")}:[/]");
            ...
            foreach (var assignment in assignments)
            {
                ...
                var projectClient = project.ClientId.HasValue
                    ? await GetClientAsync(client, project.ClientId.Value)
                    : null;
                var duration = assignment.Allocation is null
                    ? "-"
                    : assignment.Allocation.Value.TotalHours.ToString("0.#") + "h";
    ...
    {
        var confirmed = AnsiConsole.Confirm("[#F77726]Hello! Would you like to see the Forecast?[/]");
    ...
    private static async ValueTask<Client> GetClientAsync(IForecastClient forecastClient, ForecastClientId id)
    {
        var key = $"client:{id}";

        return await cache.GetOrCreateAsync(key, async _ => await forecastClient.GetClientAsync(id)) ??
               throw new InvalidOperationException("Failed to fetch the specified client");
    }

    private static async ValueTask<Project> GetProjectAsync(IForecastClient forecastClient, ForecastProjectId id)
    {
        var key = $"project:{id}";
